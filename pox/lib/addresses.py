...
class IPV6Addr(object):
    def __init__(self,addr):
        if isinstance(addr,basestring) or isinstance(addr,bytes):
            self._value = struct.unpack('dd', socket.inet_pton(addr[:8]),socket.inet_pton(addr[8:]))[0]
        elif isinstance(addr,IPV6Addr):
            pass
        else:
            raise RuntimeError("Bad format for IPv6 address")
