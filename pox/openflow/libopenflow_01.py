...
from pox.lib.packet.ooe import ooe    #shengrulee
...
ofp_flow_wildcards_rev_map = {
  ...
  'OFPFW_NW_TOS'       : 1 << 21,
  'OFPFW_OOE_DST'      : 1 << 22,
  'OFPFW_OOE_START'    : 1 << 23,
  'OFPFW_OOE_LENGTH'   : 1 << 24,
}
...
class ofp_match (ofp_base):
  ...
  def from_packet (cls, packet, in_port = None):
    ...
    elif isinstance(p, ooe):    #shengrulee
        match.ooe_dst = p.ooe_dst
        match.ooe_start = p.ooe_start
        match.ooe_length = p.ooe_length
        match.ooe_bit_rate = p.ooe_bit_rate
        match.ooe_timeout = p.ooe_timeout
  ...
  def pack (self, flow_mod=False):
    ...
    packed = b""
    if self.adjust_wildcards and flow_mod and self.dl_type !=0x0908:
    ...
    def check_ooe(val):
      return (val or 0) if self.dl_type == 0x0908 else 0
    def check_tp(val):
      return (val or 0) if self.dl_type == 0x0800 \
    ...
    if self.dl_type == 0x0908: 
        print(self.dl_type)
        print(self.ooe_dst)
        print(self.ooe_start)
        print(self.ooe_length)  
        print("0908")                                    #shengrulee
        packed += struct.pack("!HLLL", self.dl_type or 0, 
                              check_ooe(self.ooe_dst), 
                              check_ooe(self.ooe_start), 
                              check_ooe(self.ooe_length))
    ...
    #if self.dl_type == 0x0908:                                     
    packed += struct.pack("!LLL", check_ooe(self.ooe_dst), 
                              check_ooe(self.ooe_start), 
                              check_ooe(self.ooe_length)) 
    packed += _PAD4 #shengrulee   
  ...
  def _wire_wildcards(self, wildcards):
    ...
    #elif self.dl_type == 0x0908:
        #OOE:shengrulee
        #print("_wire_wildcard:%d " % (wildcards & ~( OFPFW_OOE_DST | OFPFW_OOE_START | OFPFW_OOE_LENGTH)))
        #return wildcards & ~( OFPFW_OOE_DST | OFPFW_OOE_START | OFPFW_OOE_LENGTH)
    else:
  ...
  def show (self, prefix=''):
    ...
    outstr += append('ooe_dst')    #shengrulee
    outstr += append('ooe_start')
    outstr += append('ooe_length')
...
@openflow_action('OFPAT_SET_OOE_DST', 12)  #shengrulee
@openflow_action('OFPAT_SET_OOE_START', 13)
@openflow_action('OFPAT_SET_OOE_LENGTH', 14)
class ofp_action_ooe(ofp_action_base):
    @classmethod
    def set_dst(cls, ooe = None):
        return cls(OFPAT_SET_OOE_DST, ooe)
    @classmethod
    def set_start(cls, ooe = None):
        return cls(OFPAT_SET_OOE_START, ooe)
    @classmethod
    def set_length(cls, ooe = None):
        return cls(OFPAT_SET_OOE_LENGTH, ooe)
    
    def __init__(self, type = None, ooe = None):
        """
        'type' should be OFPAT_SET_OOE_DST or OFPAT_SET_OOE_START or OFPAT_SET_OOE_LENGTH
        """
        self.type = type
        self.ooe = ooe
    ...
    def pack(self):
        ...
        packed = b""
        packed += struct.pack("!HHL", self.type, len(self), int(self.ooe))
        #packed += _PAD2
        return packed
        
    def unpack(self,raw,offset=0):
        _offset = offset
        offset,(self.type, length, self.ooe) = \
        _unpack("!HHL", raw, offset)
        offset = _skip(raw, offset, 2)
        assert offset - _offset == len(self)
        return offset
    ...
    def __eq__ (self, other):
        ...
        if self.type != other.type: return False
        if len(self) != len(other): return False
        if self.ooe != other.ooe: return False
        return True    
    ...
    def show(self,prefix=''):
        ...
        outstr += prefix + 'type: ' + str(self.type) + '\n'
        outstr += prefix + 'len: ' + str(len(self)) + '\n'
        outstr += prefix + 'ooe: ' + str(self.ooe) + '\n'
        return outstr        
...
ofp_match_data = {
  ...
  'ooe_dst': (0, OFPFW_OOE_DST),
  'ooe_start': (0, OFPFW_OOE_START),
  'ooe_length': (0, OFPFW_OOE_LENGTH),
}
